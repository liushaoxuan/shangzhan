...
import com.wyu.iwork.view.activity.CardBagActivity;
...
public class CardBagAdapter extends RecyclerView.Adapter<CardBagAdapter.ViewHolder> {
    ...
    private ArrayList<CardModel.Data.Card> datasList;
    private LayoutInflater mInflater;

    public CardBagAdapter(Context context, ArrayList<CardModel.Data.Card> datasList){
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        try {
            checkStr(datasList.get(position).getName().substring(0,1),holder.card_avatar);
            checkStr(datasList.get(position).getName(),holder.card_name);
            checkStr(datasList.get(position).getPhone(),holder.card_phone);
            checkStr(datasList.get(position).getCompany(),holder.card_company);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    private void deleteCard(final int id){
        ...
        String Sign = Md5Util.getSign(F+V+RandStr+  MyApplication.userInfo.getUser_id()+datasList.get(id).getId());
        HashMap<String,String> data = new HashMap<>();
        data.put("user_id",  MyApplication.userInfo.getUser_id());
        data.put("card_id",datasList.get(id).getId());
        data.put("F",F);
        data.put("V",V);
                    ...
                    public void onSuccess(String s, Call call, Response response) {
                        ...
                        try {
                            ...
                            if("0".equals(object.getString("code"))){
                                ...
                                if(datasList.size() == 0){
                                    ((CardBagActivity)context).showContentImpl(false);
                                }
                            }
    ...
    private void startDetailPage(int id){
        ...
        detailIntent.putExtra("id",datasList.get(id).getId());
